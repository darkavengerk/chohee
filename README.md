...
AI로 만든 음악을 공유하는 스트리밍 서비스이자, 사용자가 가사만 올리면 운영자(또는 AI)가 음악을 붙여주는 독자적 공간. 앨범으로 통일성 있는 컨셉도 묶을 수 있다.

세 가지 콘텐츠가 공존한다:

1. **트랙** — 오디오 + 가사가 모두 있는 완성곡
2. **가사** — 텍스트만 있는 1급 콘텐츠. 음악으로 변환 대기 가능
...
5. `/upload/lyrics`에서 가사 작성 + 음악 생성 요청 → `music_generation_requests` 레코드 생성
...
- `apps/admin` 운영자 어드민 — 음악 생성 요청 큐 처리
