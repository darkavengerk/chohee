...
- **Cloudflare 생태계 중심**: Workers + D1 + R2 + Pages로 비용과 운영 부담 최소화. Web 호스팅도 Cloudflare Pages.
- **SvelteKit 채택**: 초기 Next.js 14 셋업에서 Cloudflare Pages 호환성 문제 발생. SvelteKit + `@sveltejs/adapter-cloudflare` 조합으로 1급 호환을 확보. (2026-05-19 결정)
...
- **어댑터**: `@sveltejs/adapter-cloudflare` → Cloudflare Pages Functions
...
```
apps/
  web/          # SvelteKit (adapter-cloudflare로 Pages 빌드)
  api/          # Cloudflare Workers + Hono
  admin/        # Phase 2부터 운영자 어드민 (SvelteKit)
packages/
  shared/       # 공유 타입, 상수, 유틸
...
**목표**: 사용자가 가입/로그인하고, 웹에서 곡/가사/앨범을 올릴 수 있다. 디자인 시스템이 처음부터 적용된다. SvelteKit 기반.

### 범위

**인증과 사용자 관리**
- 모노레포 셋업 (Turborepo, pnpm workspaces)
- SvelteKit + `@sveltejs/adapter-cloudflare` 앱 부트스트랩
- 카카오 OAuth + 자체 JWT (access 1h, refresh 30d, httpOnly 쿠키)
- 다중 OAuth 확장 가능한 스키마 (users, auth_providers, refresh_tokens)
- 보호된 라우트 (`hooks.server.ts`에서 cookie 검증 + redirect)
- 프로필 페이지와 수정 기능 (`+page.server.ts` load + `actions`)

**디자인 시스템 통합**
- 별도 디자인 작업물의 토큰(색상, 타이포, 간격, 모서리, 그림자, 모션)을 `packages/ui`로 통합
- `packages/ui`에 Svelte 컴포넌트로 재작성 (Button, Input, Card, Badge, StatusBadge, Chip, Avatar, ProgressBar, DropZone, Tabs, Toast, Dialog, Lyrics, Waveform, Sidebar, TopBar, CoverArt, SongCard/LyricsCard/AlbumCard, FooterPlayer)
- Tailwind preset에 토큰 매핑 (slash-alpha 호환을 위해 `oklch(from var(--token) l c h / <alpha-value>)` 형식)
- 폰트 로딩 (Pretendard, Noto Serif KR, JetBrains Mono)
- 다크 모드 일관성 확인
...
- `src/routes/login/+page.svelte` + `+page.server.ts` — 카카오 로그인 진입
- `src/routes/auth/kakao/callback/+server.ts` — OAuth callback endpoint (Workers API에 code 전달 후 same-origin cookie 재발급)
- `src/routes/me/+page.svelte` + `+page.server.ts` — 마이페이지 (load로 본인 콘텐츠 로드)
- `src/routes/me/edit/+page.svelte` + `+page.server.ts` + form actions
- `src/routes/upload/+page.svelte` — 업로드 진입 ("무엇을 올릴까요?")
- `src/routes/upload/track/+page.svelte` — 곡 업로드 (ffmpeg.wasm worker 통합)
- `src/routes/upload/lyrics/+page.svelte` — 가사 작성 + 음악 제안 의향 발행
- `src/routes/upload/album/+page.svelte` — 앨범 생성
- `src/routes/discover/+page.svelte` — 탐색 (Phase 2에서 풍부화)
- `src/hooks.server.ts` — 모든 요청에 cookie 검사 + 보호 라우트 redirect
- `src/lib/server/api.ts` — API 프록시 헬퍼 (서버 환경에서 Workers API 호출)
- `src/lib/api-client.ts` — 클라이언트 fetch (가능한 한 SvelteKit endpoint 경유)
...
- `pnpm --filter @chohee/web dev` (Vite dev server, 기본 :5173)
- 환경 변수 관리 (`.env.example` 커밋)
- 루트 `pnpm dev`로 web/api 동시 실행 (Turborepo)
- 로컬 dev-login 엔드포인트 유지 (카카오 없이도 보호 페이지 진입 가능)
...
- 한국어 텍스트 가독성 확보
- Cloudflare Pages 배포가 안정적으로 성공 (빌드 호환성 이슈 없음)

### 기술 메모

- ffmpeg.wasm: SharedArrayBuffer 필요 → COOP/COEP 헤더 설정 (`src/hooks.server.ts`의 `handle`에서 `setHeaders`)
- 매우 긴 곡(15분+)은 메모리 주의 — 청크 처리 검토
- R2 객체 키 구조: `tracks/{user_id}/{track_id}/{bitrate}.m4a`, `tracks/{user_id}/{track_id}/waveform.json`
- 자주 변하는 세부 사항(인코딩 상세, R2 키 구조, 카카오 OAuth 자세한 흐름, SvelteKit load/actions 패턴)은 스킬로 분리
- SvelteKit의 서버 endpoint(`+page.server.ts`/`+server.ts`)에서 Workers API를 fetch — 같은 origin 처럼 동작하여 cookie 흐름 단순화
...
**콘텐츠 페이지**
- 곡 상세 페이지 (`src/routes/songs/[id]/+page.svelte`)
  - 큰 커버 아트, 메타데이터, 설명
  - 가사 표시 영역 (1급 시민, 타이포그래피 신중하게)
  - 파형 플레이어 (재생/일시정지/시킹/시간)
  - 가사가 연결된 트랙이면 "이 곡은 「가사 제목」의 채택된 음원입니다" 컨텍스트와 작성자/제안자 크레딧
  - 같은 앨범/같은 작가의 다른 곡
- 가사 상세 페이지 (`src/routes/lyrics/[id]/+page.svelte`, 음악 없는 가사 또는 제안 모집 중인 가사)
  - 가사 텍스트가 메인 콘텐츠
...
- 앨범 페이지 (`src/routes/albums/[id]/+page.svelte`)
  - 컨셉/무드 정보
  - 수록 곡과 가사를 함께 표시
  - 앨범 전체 재생
- 창작자 프로필 페이지 (`src/routes/u/[handle]/+page.svelte`)
  - 프로필 정보
...
- 글로벌 푸터 플레이어 — Svelte store (`$lib/stores/player.ts`)로 페이지 전환 시 재생 유지
...
- 별도 SvelteKit 앱으로 분리
- 관리자 권한 미들웨어 (`hooks.server.ts`에서 isAdmin 검사)
...
- 페이지를 오가며 재생이 끊기지 않음 (Svelte store 기반 글로벌 player)
...
- 글로벌 푸터 플레이어 — SvelteKit의 `+layout.svelte`에 두고 `$lib/stores/player.ts` writable로 관리. 페이지 전환 시 컴포넌트는 유지되니 audio 객체도 끊기지 않음.
...
- 다국어 UI 기반 마련 (i18n 키 구조 — SvelteKit의 `paraglide` 또는 자체 키 관리)
...
- **2026-05-18 ~ 2026-05-19**: Next.js 14 App Router로 Phase 1 스캐폴드 완료. Cloudflare Pages 배포 시도 중 `@cloudflare/next-on-pages` 어댑터의 webpack 호환 문제로 빌드 fail 반복. Next 15 + React 19 업그레이드, Vercel 전환 등 검토 후 **SvelteKit + adapter-cloudflare**로 갈아타기로 결정.
- **2026-05-19**: Phase 1을 SvelteKit 기반으로 재시작. API(Workers), DB(D1), 디자인 토큰은 그대로 유지. 웹 UI 코드는 재작성.
